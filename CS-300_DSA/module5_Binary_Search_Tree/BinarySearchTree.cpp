...
#include <iostream>
#include <time.h>
#include <algorithm> //needed for std::remove method
#include <cstdlib>
...
Bid BinarySearchTree::Search(string bidId) {
    ...
    //initialize new empty object if no match found
    Bid bid;
    // inform user no match found
    cout << bidId << " not found.\n";
    //return empty object
    return bid;
}
...
void BinarySearchTree::inOrder(Node* node) {
    ...
    if (BinarySearchTree::root == nullptr)
        {
            cout << "I'm sorry, tree is empty." << endl;
            return;
        }
...
Node* BinarySearchTree::removeNode(Node* node, string bidId) {
    ...
    //base case: if node is nullptr, indicating end of branch reached and no match found
    if (node == nullptr)
    {
        cout << "I'm sorry, node not found." << endl;
        return nullptr;
    }
    // (otherwise recurse down the left subtree)
    if (node-> bid.bidId > bidId)
    {
        // remove left node using recursive call
    ...
    // remove right node using recursive call
        root-> right = removeNode(node-> right, tempNode-> bid.bidId);
    }
    // return node
    return node;
}
